...
public class MyDatePickerDialog extends AlertDialog implements OnClickListener, OnDateChangedListener {
    ...
    public MyDatePickerDialog(Context context, OnDateSetListener callBack, int year, int monthOfYear, int dayOfMonth,int type) {
        this(context, 0, callBack, year, monthOfYear, dayOfMonth, type);
    }


    public MyDatePickerDialog(Context context, int theme, OnDateSetListener callBack, int year, int monthOfYear,
                              int dayOfMonth, int type) {
        super(context, theme);
        ...
        if(type==0) hideDay(mDatePickerStart);
        else if (type==1) hideDayAndMonth(mDatePickerStart);
    }

    private void hideDayAndMonth(DatePicker mDatePicker){
        try {
